...
class nusoap_fault extends MyNuSoap {
	...
	*/
	function nusoap_fault_function($faultcode,$faultactor='',$faultstring='',$faultdetail=''){
		parent::MyNuSoapFunc();
...
class nusoap_xmlschema extends MyNuSoap  {
	...
	* @access   public
	*/
	function nusoap_xmlschema_func($schema='',$xml='',$namespaces=array()){
		parent::MyNuSoapFunc();
...
class soapval extends MyNuSoap {
	...
	* @access   public
	*/
  	function soapvalfunc($name='soapval',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false) {
		parent::MyNuSoapFunc();
		$this->name = $name;
		$this->type = $type;
		$this->value = $value;
...
class soap_transport_http extends MyNuSoap {
	...
	* @access public
	*/
	function soap_transport_http_func($url, $curl_options = NULL, $use_curl = false){
		parent::MyNuSoapFunc();
...
class nusoap_server extends MyNuSoap {
	...
	* @access   public
	*/
	function nusoap_server_func($wsdl=false){
		parent::MyNuSoapFunc();
		// turn on debugging?
		global $debug;
	...
	*/
    function parseRequest($headers, $data) {
		...
		$parser = new nusoap_parser_func($data,$this->xml_encoding,'',$this->decode_utf8);
		// parser debug
	...
	*/
	function fault($faultcode,$faultstring,$faultactor='',$faultdetail=''){
		...
		$this->fault = new nusoap_fault_function($faultcode,$faultactor,$faultstring,$faultdetail);
    ...
    */
    function configurewsdl_func($serviceName,$namespace = false,$endpoint = false,$style='rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
    	global $HTTP_SERVER_VARS;
        ...
        $this->wsdl->schemas[$schemaTargetNamespace][0] = new nusoap_xmlschema_func('', '', $this->wsdl->namespaces);
        if ($style == 'document') {
...
class wsdl extends MyNuSoap {
     ...
     * @access public 
     */
    function wsdl_func($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=30,$curl_options=null,$use_curl=false){
		parent::MyNuSoapFunc();
	 ...
	 * @access public
	 */
	function fetchwsdl_func($wsdl) {
		$this->debug("parse and process WSDL path=$wsdl");
		$this->wsdl = $wsdl;
        // parse wsdl file
        if ($this->wsdl != "") {
            $this->parsewsdl_func($this->wsdl);
        }
        // imports
     ...
     * @access private 
     */
    function parsewsdl_func($wsdl = '') {
		$this->debug("parse WSDL at path=$wsdl");

        if ($wsdl == '') {
            $this->debug('no wsdl passed to parsewsdl_func()!!');
            $this->setError('no wsdl passed to parsewsdl_func()!!');
            return false;
        }
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
        	...
        	// get wsdl
	        $tr = new soap_transport_http_func($wsdl, $this->curl_options, $this->use_curl);
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->status = 'schema';
            $this->currentSchema = new nusoap_xmlschema_func('', '', $this->namespaces);
            $this->currentSchema->schemaStartElement($parser, $name, $attrs);
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
        } else {
...
class nusoap_parser extends MyNuSoap {
	...
	* @access   public
	*/
	function nusoap_parser_func($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		parent::MyNuSoapFunc();
		$this->xml = $xml;
		...
		if(!empty($xml)){
			...
			$this->debug('Entering nusoap_parser_func(), length='.strlen($xml).', encoding='.$encoding);
...
* $soapclient = new NuSoapClientFunc( string path [ ,mixed wsdl] );
*
...
* @access   public
*/
class NuSoapClient extends MyNuSoap  {
	...
	* @access   public
	*/
	function NuSoapClientFunc($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $portName = ''){
		parent::MyNuSoapFunc();
	...
	*/
	 ...
	 * @access	public
	 */
	function loadwsdl_func() {
		$this->debug('instantiating wsdl class with doc: '.$this->wsdlFile);
		$this->wsdl = new wsdl_func('',$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,$this->timeout,$this->response_timeout,$this->curl_options,$this->use_curl);
		$this->wsdl->setCredentials($this->username, $this->password, $this->authtype, $this->certRequest);
		$this->wsdl->fetchwsdl_func($this->wsdlFile);
		$this->checkwsdl_func();
	}
		...
		switch(true){
			...
			case preg_match('/^http/',$this->endpoint):
				...
				if($this->persistentConnection == true && is_object($this->persistentConnection)){
					...
					$http = new soap_transport_http_func($this->endpoint, $this->curl_options, $this->use_curl);
					if ($this->persistentConnection) {
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
		$parser = new nusoap_parser_func($data,$this->xml_encoding,$this->operation,$this->decode_utf8);
